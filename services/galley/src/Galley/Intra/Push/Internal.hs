...
data PushTo user = Push
  { _conn :: Maybe ConnId,
    _transient :: Bool,
    _route :: Gundeck.Route,
    _nativePriority :: Maybe Gundeck.Priority,
    _async :: Bool,
    origin :: Maybe UserId,
    _recipients :: List1 (RecipientBy user),
    json :: Object,
    recipientListType :: ListType
  }
  deriving stock (Functor, Foldable, Traversable, Show)
...
push ps = do
  ...
  where
    ...
    mkPushTo p =
      nonEmpty (toList (_recipients p)) <&> \nonEmptyRecipients ->
        p {_recipients = List1 nonEmptyRecipients}
...
pushLocal ps = do
  ...
  let (asyncs, syncs) = partition _async (removeIfLargeFanout limit $ toList ps)
  ...
  where
    ...
    chunk n acc (y : ys)
      ...
      | otherwise =
          let totalLength = (n + length (_recipients y))
    ...
    splitPush n p =
      let (r1, r2) = splitAt n (toList (_recipients p))
       in (p {_recipients = fromJust $ maybeList1 r1}, p {_recipients = fromJust $ maybeList1 r2})

    maxRecipients :: Int
    maxRecipients = 128

    recipientList :: PushTo UserId -> [Gundeck.Recipient]
    recipientList p = map (toRecipient p) . toList $ _recipients p

    toPush :: PushTo user -> [Gundeck.Recipient] -> Gundeck.Push
    toPush p r =
      let pload = Gundeck.singletonPayload (json p)
       in Gundeck.newPush (origin p) (unsafeRange (Set.fromList r)) pload
            & Gundeck.pushOriginConnection .~ _conn p
            & Gundeck.pushTransient .~ _transient p
            & maybe id (set Gundeck.pushNativePriority) (_nativePriority p)

    toRecipient :: PushTo user -> RecipientBy UserId -> Gundeck.Recipient
    toRecipient p r =
      Gundeck.recipient (_userId r) (_route p)
        & Gundeck.recipientClients .~ _clients r
    ...
    removeIfLargeFanout limit =
      filter
        ( \p ->
            (recipientListType p == ListComplete)
              && (length (_recipients p) <= fromIntegral (fromRange limit))
        )
...
newPush1 recipientListType from e rr =
  Push
    { _conn = Nothing,
      _transient = False,
      _route = Gundeck.RouteAny,
      _nativePriority = Nothing,
      _async = False,
      recipientListType = recipientListType,
      json = pushEventJson e,
      origin = from,
      _recipients = rr
    }
